...
/**
 * Created by Jerry Lin on 2016/1/1.
 */
public class algorithm_test {
    public ArrayList<Event> Schedule(ArrayList<Event> OriginData) {
        ...
        for (int i = 0; i < WaitForSchedule.size(); i++)
        {
            ...
            WaitForSchedule.get(i).setPriority(totaltime / doeventtime);
        }
            ...
            for (int i = 6; i < 24; i += 1) {//最早從六點開始排
                if (Hour[i] == true)
                {
                    ...
                    {//找優先度最高的事件 同時是可以做的 紀錄優先度最高事件的ID
                        ...
                        {// 只判斷可做的時間到了沒     還沒判斷是不是超過結束時間 &&EndTime.after(Time)
                            if (ChosenPriorityId == -1 || Temp.getPriority() > WaitForSchedule.get(index).getPriority()) //判斷優先度
                            {
                    ...
                    for(j=i+1;j<24;j+=1)
                    {
                        ...
                        for (int k = 0; k < WaitForSchedule.size(); k++)
                        {
                            ...
                            if (now.before(Time) && WaitForSchedule.get(k).getDoHours() > 0)
                            {
                                ...
                                WaitForSchedule.get(k).setPriority(totaltime / doeventtime);
